...
def compute_one(m, fun, fun_der_sec):
    ...
    res = (b1 + b2) * rf / (fr ** 2) + fun(1.0, theta)
    return res

@tf.function
def get_up(m):
    return compute_one(m, fun=up_fun, fun_der_sec=up_fun_der_sec)

@tf.function
def get_down(m):
    return compute_one(m, fun=down_fun, fun_der_sec=down_fun_der_sec)
...
def custom_loss(y_true, y_pred):
    ...
    up = tf.map_fn(fn=get_up, elems=y_pred)
    down = tf.map_fn(fn=get_down, elems=y_pred)
    p = up/down
    return tf.losses.MSE(y_true, p)

m = np.concatenate([y.reshape(-1,1),other_X],1)
m = m[0,:]
fun = up_fun
fun_der_sec = up_fun_der_sec
...
def get_pred(x):
    ...
    up = tf.map_fn(fn=get_up, elems=pred)
    down = tf.map_fn(fn=get_down, elems=pred)
    p = up/down
    return p


pred=model.predict([X, other_X])
pred=get_pred([X, other_X])
y
pred[:,-1]
